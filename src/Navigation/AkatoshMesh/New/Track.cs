...
-> 1 will only consider the real cost without estimating the remaining cost.");
        ...
        public Track(Track PreviousTrack, Node TransitionNode) {
            if (_Target == null)
                throw new InvalidOperationException("You must specify a target Node for the Track class.");
            Queue = PreviousTrack;
            _Cost = Queue.Cost + 1;
            if (PreviousTrack != null && PreviousTrack.EndNode != null && TransitionNode != null) {
                var distance = PreviousTrack.EndNode.Position.Distance(TransitionNode.Position);
                _Cost += distance;
            }

            if (!(TransitionNode is AkatoshGroundNode)) {
                _Cost += 4;
            }

            _NbArcsVisited = Queue._NbArcsVisited + 1;
            EndNode = TransitionNode;
        }
		...
		public static bool SameEndNode(object O1, object O2)
		{
			...
			return Equals(P1.EndNode, P2.EndNode);
		}
	}
}
