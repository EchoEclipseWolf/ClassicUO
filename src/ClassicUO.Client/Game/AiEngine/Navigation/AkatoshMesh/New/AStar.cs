...
using AkatoshQuester.Helpers.Collections;
...
namespace AkatoshQuester.Helpers.Cartography
{
	...
	/// <summary>
	/// Class to search the best path between two nodes on a MeshGraph.
	/// </summary>
	public class AStar
	{
		...
		/// AStar Constructor.
		/// </summary>
		/// <param name="G">The MeshGraph on which AStar will perform the search.</param>
		public AStar()
		{
            _meshGraph = Navigation.CurrentMesh;
		...
		public bool SearchPath(Node StartNode, Node EndNode, int maxSearch = 50000) {
            int count = 0;
			lock (_meshGraph)
			{
            ...
            foreach (var A in TrackToPropagate.EndNode.Linked) {
                try {
                    ...
                    if (node.Passable) {
                        ...
                        SearchedIds.Add(Successor.EndNode.Id);
                        _Open.Add(Successor);
                    }
                } catch (Exception e) {
                    continue;
